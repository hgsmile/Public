...
public class AudioTaskHandler {
  ...
  private void cutAudio(String srcPath, float startTime, float endTime){
    ...
    String outName = nameNoSuffix + "_cut.pcm";

    //裁剪后音频的路径
    String destPath = Environment.getExternalStorageDirectory().getPath() + File.separator + outName;
  ...
  private void decodeAudio(String path, String destPath){
    ...
    DecodeEngine.getInstance().convertMusicFileToPcmFile(path, destPath, new DecodeOperateInterface() {
